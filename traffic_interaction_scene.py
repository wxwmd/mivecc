...
class MultiTrafficInteraction:
    ...
    def __init__(self, arrive_time, args, m_num=3, dis_ctl=150, deltaT=0.1, vm=5, vM=13, am=-3, aM=3, v0=10,
        ...
        self.ogm_total_s = np.zeros([pow(m_num, 2), self.args.s_agent_num, self.args.s_agent_num, 3]) # 9个力扣，每个路口挑15个车，15*15的状态矩阵有三个：[位置，速度，加速度]
    ...
    def scene_update(self, count=0):
        ...
        self.ogm_total_s = np.zeros([pow(self.m_num, 2), self.args.s_agent_num, self.args.s_agent_num, 3]) #[9,15,15,3]
        self.ogm_total_m = np.zeros([self.args.m_agent_num, self.args.m_agent_num, 3]) # [60,60,3]
        self.reward_total_s = np.zeros(pow(self.m_num, 2)) # [9]
    ...
    # 根据给定的action（edge端给的动作），m_action：全局动作，更新车辆状态
    def step(self, action, m_action, train_flag):
        v = []
        a = []
        num_total = 0
        co_num = 0
        self.virtual_lane = [[] for i in range(pow(self.m_num, 2))] # 9
        for i in range(self.lane_num): # 12个道
            for j, item in enumerate(self.veh_info[i]): # 遍历这条道上的所有车辆信息
                choose = False
                num_total += 1
                ...
                v_new = min(self.vM,
                            max(self.veh_info[i][j]["v"] + self.veh_info[i][j]["a"] * self.deltaT,
                                self.vm))  # 计算新速度
                if v_new != self.veh_info[i][j]["v"]: # 更新车辆到达碰撞点的位置
